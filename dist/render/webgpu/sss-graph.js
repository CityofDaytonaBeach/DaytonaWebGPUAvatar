...
export class SssRenderGraph {
    ...
    tanHalfFov) {
        ...
        this.paramBuffers = SSS_PASS_DIRECTIONS.map(() => device.createBuffer({ size: 16, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST }));
    }
    ...
    resize(width, height) {
        ...
        const tex = (format, label) => this.device.createTexture({ size: { width: w, height: h }, format, usage, label });
        this.targets = {
            width: w,
            height: h,
    ...
    run(encoder, outputView) {
        ...
        for (let i = 0; i < passes.length; i++) {
            ...
            this.device.queue.writeBuffer(this.paramBuffers[i], 0, sssParamsData(SSS_PASS_DIRECTIONS[i], this.tanHalfFov, viewportPixels));
    ...
    requireTargets() {
        if (!this.targets)
            throw new Error('SssRenderGraph: call resize(width, height) first');
        return this.targets;
    }
}
//# sourceMappingURL=sss-graph.js.map