...
export class WebGL2HumanRenderer {
    ...
    constructor(canvas, canonical) {
        ...
        if (!mvpLoc || !normalLoc || !colorLoc)
            throw new Error('WebGL2 shader uniform missing');
        this.mvpLoc = mvpLoc;
...
export function buildWebGL2RenderParts(canonical) {
...
export function webglPartColor(name, kind) {
    if (kind === 'sclera')
        return [0.95, 0.95, 0.95];
    if (kind === 'limbus')
        return [0.3, 0.34, 0.32];
    if (kind === 'cornea')
        return [0.55, 0.6, 0.62];
    if (kind === 'iris')
        return name.startsWith('pupil') ? [0.12, 0.1, 0.12] : [0.35, 0.52, 0.38];
    if (kind === 'teeth')
        return [0.93, 0.91, 0.84];
    if (kind === 'tongue')
        return [0.82, 0.5, 0.48];
    if (kind === 'mouth_cavity')
        return [0.22, 0.1, 0.11];
    return [0.72, 0.56, 0.45];
}
function extractUvs(canonical) {
