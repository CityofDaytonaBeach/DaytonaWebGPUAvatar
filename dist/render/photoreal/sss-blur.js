 ...
 */
export const SSS_FALLOFF = [1, 0.37, 0.19];
/** Sum of two Gaussians — the classic 2-lobe fit to the dipole diffusion profile. */
export function diffusionProfile(x, falloff) {
    const f = Math.max(falloff, 1e-4);
    const g = (v, variance) => Math.exp(-(v * v) / (2 * variance)) / Math.sqrt(2 * Math.PI * variance);
 ...
 */
export function sssKernel(taps = C.sssBlurTaps) {
 ...
 */
export function sssStepUV(depth, tanHalfFov, viewportPixels, width = C.sssBlurWidth) {
...
export function depthRejection(centerDepth, sampleDepth) {
 ...
 */
export function sssBlurPass(center, taps, kernel) {
    let out = [0, 0, 0];
...
export function sssBlurWgsl(opts = {}) {
 ...
 */
export function sssParamsData(direction, tanHalfFov, viewportPixels) {
    return new Float32Array([direction[0], direction[1], tanHalfFov, viewportPixels]);
}
//# sourceMappingURL=sss-blur.js.map