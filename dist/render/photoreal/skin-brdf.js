...
import { clamp01, toDisplay, vadd, vdot, vlerp, vmul, vnormalize, vscale } from './color.js';
const PI = Math.PI;
...
export function distributionGGX(ndh, roughness) {
 ...
 */
export function visibilitySmithCorrelated(ndv, ndl, roughness) {
    const a = Math.max(roughness * roughness, PHOTOREAL_CONSTANTS.minRoughness ** 2);
    const a2 = a * a;
 ...
 */
export function dualLobeSpecular(ndh, ndv, ndl, roughness) {
 ...
 */
export function preIntegratedScatter(ndl, curvature, scatterColor, intensity) {
    ...
    const scattered = [red * scatterColor[0], green * scatterColor[1], blue * scatterColor[2]];
 ...
 */
export function transmission(normal, lightDir, viewDir, thickness, scatterColor) {
...
export function shadeSkinLight(surface, light) {
    ...
    const spec = vscale(F, specMag * Math.max(ndl, 0));
...
export function shadeSkinLinear(surface, rig = PHOTOREAL_LIGHT_RIG) {
    let total = [0, 0, 0];
    for (const light of [rig.key, rig.fill, rig.rim]) {
        total = vadd(total, shadeSkinLight(surface, {
...
export function shadeSkin(surface, rig = PHOTOREAL_LIGHT_RIG) {
    return toDisplay(shadeSkinLinear(surface, rig));
}
//# sourceMappingURL=skin-brdf.js.map