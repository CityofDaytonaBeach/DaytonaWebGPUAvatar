...
export class CorrectiveBatch {
     ...
     */
    toAtomicEvent(source = 'developer') {
...
export class ValidationCache {
     ...
     */
    snapshotChanged(definition) {
        const next = definition.serialize();
        let changed = !this.validKinds.add('__init') || this.validKinds.size === 0;
        let any = false;
        for (const [k, v] of Object.entries(next)) {
            if (this.fingerprint[k] !== v)
                any = true;
...
export class PerceptualValidator {
    ...
    run(validate) {
        ...
        for (const issue of issues) {
            if (issue.correctiveRequest)
                batch.add(issue.correctiveRequest);
        }
        return { issues, batch };
    }
    /** Aggregate severity threshold rules into a report. */
    validate(definition, canonical, dims) {
        ...
        const score = issues.length === 0 ? 1 : issues.reduce((sum, issue) => sum + issue.score, 0) / issues.length;
    ...
    validateAnatomy(issues, dims) {
        if (this.cache.isValid('anatomy.proportion'))
            return;
        for (const c of validateAnatomy(dims)) {
            if (this.anatomyOk(c))
                continue;
            const changes = {};
            if (c.message.includes('waist'))
                changes['body.waist'] = 1.0;
            if (c.message.includes('hips'))
                changes['body.hips'] = 1.0;
            issues.push({
                kind: 'anatomy.proportion',
                severity: c.satisfaction < 0.5 ? 'error' : 'warning',
                message: c.message,
                score: c.satisfaction,
                correctiveRequest: Object.keys(changes).length > 0
    ...
    validateEye(issues, definition, canonical) {
        ...
        if (eyeSpacing < this.config.minEyeSpacing ||
            eyeSpacing > this.config.maxEyeSpacing ||
            yError > this.config.eyeYErrorWarning) {
            issues.push({
                kind: 'eye.alignment',
                severity: eyeSpacing < this.config.eyeSpacingErrorMin ||
    ...
    validateMouth(issues, definition) {
        if (this.cache.isValid('mouth.intersection'))
            return;
    ...
    validateExpression(issues, definition) {
        if (this.cache.isValid('expression.range'))
            return;
    ...
    validateSkinTone(issues, definition) {
        if (this.cache.isValid('skin.tone'))
            return;
        ...
        if ([r, g, b, pigmentation].every((v) => Number.isFinite(v))) {
            ...
            if (!plausible) {
                const severity = spread > this.config.skinLuminanceSpreadMax * 3 ? 'warning' : 'info';
                issues.push({
                    kind: 'skin.tone',
                    severity,
    ...
    validateProportionRatios(issues, definition, dims) {
        if (this.cache.isValid('proportion.ratio'))
            return;
        ...
        if (height > 0) {
            ...
            if (err > this.config.armSpanRatioTolerance) {
                const severity = err > this.config.armSpanRatioTolerance * 2 ? 'warning' : 'info';
                issues.push({
        ...
        if (Number.isFinite(headScale)) {
            if (headScale < this.config.headHeightRatioMin ||
                headScale > this.config.headHeightRatioMax) {
                const severity = headScale < this.config.headHeightRatioMin * 0.9 ||
    ...
    validateSymmetry(issues, canonical) {
        if (this.cache.isValid('symmetry'))
            return;
        const pairs = [
    ...
    validateRender(issues) {
        for (const hook of this.hooks) {
            if (!hook.hasFreshFrame())
                continue;
    ...
    exportJSON(definition, canonical, dims) {
        return this.validate(definition, canonical, dims).json;
    }
}
function vertexSpread(verts) {
...
function toJSON(score, issues, severityCounts) {
    return {
        score,
        issues: issues.map((i) => ({
            kind: i.kind,
 ...
 */
export function validatePerceptualHuman(definition, canonical, dims) {
    return new PerceptualValidator().validate(definition, canonical, dims);
}
//# sourceMappingURL=perceptual-validator.js.map