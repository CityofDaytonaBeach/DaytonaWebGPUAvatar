...
import { CharacterTimeline } from './core/timeline/character-timeline.js';
import { ConstraintSolver } from './core/constraints/solver.js';
import { DependencyGraph } from './compiler/dependency/dependency-graph.js';
import { DeltaCompiler } from './compiler/delta/delta-compiler.js';
import { affectedSystemsForChange, } from './compiler/dependency/affected-systems.js';
...
import { FacialExpressionSystem, } from './animation/facial/facial-expression.js';
import { SpeechSolver, simpleTTS } from './animation/speech/speech-solver.js';
import { SemanticLOD, PerceptualLOD } from './lod/index.js';
import { DeterministicPromptInterpreter, intentToEvent } from './ai/prompt/interpreter.js';
import { WebGpuHumanPipeline } from './render/webgpu/pipeline.js';
import { resolveAnatomy, validateAnatomy, anatomySatisfaction, } from './anatomy/parametric/parametric-anatomy.js';
import { placeSkeletonFromDefinition } from './anatomy/skeleton/skeleton.js';
import { combinedSkinMatrices } from './anatomy/skeleton/bone-matrix.js';
import { SkeletalAnimation, } from './animation/skeleton/skeletal-animation.js';
import { buildInfluences, skinMeshCPU, skinNormalsCPU } from './gpu/kernels/skin-mesh.js';
import { AttachmentSystem, } from './attachments/attachment-system.js';
import { generateStrandHair, } from './surface/hair/strand-hair.js';
import { buildHumanSdfField } from './physics/sdf/human-sdf.js';
...
import { buildInternalAnatomyView, } from './anatomy/internal/internal-anatomy.js';
import { createParameterTransition, sampleTransition, transitionComplete, } from './core/transitions/parameter-transition.js';
...
export class Human {
    ...
    shapeSpace;
    correctives;
    correctiveMorphInputs = [];
    /** Vertex ids affected by the shape bases currently contributing (P17). */
    currentAffectedVertices = new Set();
    profiler = new HumanProfiler();
    facial = new FacialExpressionSystem();
    speech = new SpeechSolver();
    semanticLod = new SemanticLOD();
    perceptualLod = new PerceptualLOD();
    prompter = new DeterministicPromptInterpreter();
    animation = new SkeletalAnimation();
    motion = new MotionCompiler();
    currentPose = [];
    transitions = [];
    skinInfluences = null;
    clock = 0;
    gpu = null;
    attachments = new AttachmentSystem();
    constructor(opts) {
    ...
    registerShapeMorphsInDriver() {
        for (const basis of this.shapeSpace.bases.list()) {
            ...
            if (!isCorrective)
                this.morphDriver.registerBasis(morphName, basis.property);
        }
    ...
    skinScene() {
        const skeleton = this.parametricSkeleton();
        if (!this.skinInfluences)
            this.skinInfluences = buildInfluences(this.canonical, skeleton);
    ...
    addAttachment(attachment, source = 'api') {
    ...
    simulateCloth(mesh, steps, options = {}) {
    ...
    uploadGpu(definition = this.definition) {
        if (this.gpu)
            this.gpu.upload(definition);
    }
    /**
    ...
    applyEvent(event, opts = {}) {
        ...
        if (event.type === 'pose')
            this.applyPoseEvent(event);
        if (event.type === 'transition')
            this.registerTransitionEvent(event);
        if (event.type === 'advanceTime')
            propChanges.push(...this.applyAdvanceTimeEvent(event));
        for (const id of this.changedIdsBetween(beforeDefinition, this.definition.serialize())) {
            if (!propChanges.includes(id))
                propChanges.push(id);
        }
        ...
        const dirtyRegionNames = isAttachmentEvent(event)
            ...
            : event.type === 'pose'
                ? ['Animation']
                : this.dirty.describe();
        const verticesModified = countDirtyVertices(this.canonical, dirtyRegionNames);
    ...
    transition(path, targetValue, duration, curve = 'linear', source = 'api') {
    ...
    prompt(text, source = 'ai') {
        const intent = this.prompter.interpret(text);
        if (intent.type === 'expression') {
            return this.setExpression(intent.expression ?? 'neutral', 1);
        }
    ...
    resultFromChangedIds(changedIds) {
    ...
    applyExpressionEvent(event) {
        ...
        if (typeof expression !== 'string')
            return;
        this.facial.apply(this.definition, expression, typeof intensity === 'number' ? intensity : 1);
    }
    kernelWorkForEvent(event) {
        if (event.type === 'pose') {
            return [
                {
                    kind: 'Skinning',
    ...
    registerTransitionEvent(event) {
        ...
        if (typeof path !== 'string' || typeof duration !== 'number')
            return;
    ...
    applyActiveTransitions() {
        ...
        for (const transition of this.transitions) {
            ...
            if (this.definition.getById(id) !== before)
                changed.push(id);
            if (!transitionComplete(transition, this.clock))
                remaining.push(transition);
        }
        this.transitions = remaining;
        return changed;
    }
    rebuildTemporalStateFromTimeline() {
        ...
        for (const event of active) {
            ...
            if (event.type === 'transition')
                this.registerTransitionEvent(event);
            if (event.type === 'advanceTime')
                this.applyAdvanceTimeEvent(event);
        }
    }
}
...
function isTransitionCurve(value) {
