 ...
 */
export function buildHdShapeSpace(canonical) {
    ...
    const addFineControl = (name, property, fineRegions, coarseRegions, fn, tags) => {
        ...
        for (let i = 0; i < canonical.vertexCount; i++) {
            if (regionSet.has(canonical.vertices[i].region))
                ids.push(i);
        }
    ...
    addFineControl('NoseWidthBasis', 'face.nose.width', ['nose_alar_left', 'nose_alar_right', 'nose_tip', 'nose_bridge'], ['nose'], (vx, _vy, vz) => {
    ...
    addFineControl('NoseLengthBasis', 'face.nose.length', ['nose_tip', 'nose_alar_left', 'nose_alar_right', 'nose_bridge'], ['nose'], (_vx, _vy, vz) => ({
        dx: 0,
        dy: 0,
    ...
    addFineControl('ChinProjectionBasis', 'face.chin.projection', ['chin', 'lower_lip', 'jaw_left', 'jaw_right'], ['jaw'], (_vx, _vy, vz) => ({ dx: 0, dy: 0, dz: vz >= 0.22 ? 0.045 : 0.02 }), ['chin', 'correlated']);
    ...
    addFineControl('EyeSpacingBasis', 'face.eye.spacing', [
        'eye_left',
        'eye_right',
        'upper_eyelid_left',
        'upper_eyelid_right',
        'lower_eyelid_left',
        'lower_eyelid_right',
    ], ['eyes'], (vx, _vy, _vz) => ({ dx: Math.sign(vx || 1e-6) * 0.04, dy: 0, dz: 0 }), ['eye', 'correlated']);
    // Eye size: eyelid regions expand about the eye centers (both axes).
    addFineControl('EyeSizeBasis', 'face.eye.size', [
        'upper_eyelid_left',
        'upper_eyelid_right',
        'lower_eyelid_left',
        'lower_eyelid_right',
    ...
    addFineControl('CheekWidthBasis', 'face.cheek.width', ['cheek_left', 'cheek_right', 'jaw_left', 'jaw_right', 'temple_left', 'temple_right'], ['cheek_left', 'cheek_right'], (vx, _vy, _vz) => {
        const s = Math.sign(vx || 1e-6);
        const strength = Math.abs(vx) > 0.06 ? 0.04 : 0.028;
        return { dx: s * strength, dy: 0, dz: 0 };
    }, ['cheek', 'correlated']);
    // Mouth width: mouth corners spread, lips widen slightly.
    addFineControl('MouthWidthBasis', 'face.mouth.width', ['mouth_corner_left', 'mouth_corner_right', 'upper_lip', 'lower_lip'], ['mouth'], (vx, _vy, _vz) => {
        const s = Math.sign(vx || 1e-6);
    ...
    const correctiveBasis = (name, regions, fn) => {
    ...
    if (jawBasis && noseBasis) {
        const output = correctiveBasis('WideJawWideNoseCorrective', ['nose_alar_left', 'nose_alar_right', 'jaw_left', 'jaw_right'], (vx, _vy, _vz) => ({ dx: Math.sign(vx || 1e-6) * 0.018, dy: 0, dz: 0.005 }));
        if (output != null) {
            spec.correctiveRules.push({
    ...
    if (cheekBasis && jawBasis) {
        const output = correctiveBasis('WideCheeksWideJawCorrective', ['cheek_left', 'cheek_right', 'jaw_left', 'jaw_right'], (vx, _vy, _vz) => ({ dx: Math.sign(vx || 1e-6) * 0.02, dy: -0.004, dz: 0 }));
        if (output != null) {
            spec.correctiveRules.push({
    ...
    addFineControl('ChestWidthBasis', 'body.chest', ['chest', 'back', 'shoulder_left', 'shoulder_right'], ['torso'], (vx, _vy, vz) => {
        const s = Math.sign(vx || 1e-6);
    ...
    addFineControl('WaistBasis', 'body.waist', ['abdomen', 'chest', 'back', 'pelvis'], ['torso'], (vx, _vy, vz) => ({
        dx: vx * 0.42,
        dy: 0,
        dz: vz * 0.42,
    }), ['waist', 'correlated']);
    ...
    addFineControl('HipWidthBasis', 'body.hips', ['pelvis', 'abdomen', 'thigh_left', 'thigh_right'], ['torso'], (vx, _vy, vz) => ({
        dx: vx * 0.4,
        dy: 0,
        dz: vz * 0.4,
    }), ['hips', 'correlated']);
    ...
    addFineControl('BodyFatBasis', 'body.bodyFat', [
        'chest',
        'abdomen',
        'back',
        'shoulder_left',
        ...
        'thigh_left',
        'thigh_right',
    ], ['torso', 'upperarm_l', 'upperarm_r', 'thigh_l', 'thigh_r'], (vx, _vy, vz) => ({
        dx: vx * 0.3,
        dy: 0,
        dz: vz * 0.3,
    }), ['bodyFat', 'correlated']);
    ...
    // default 0.48.
    addFineControl('MuscleDefinitionBasis', 'body.muscularity', [
        'chest',
        'back',
        'shoulder_left',
        'shoulder_right',
    ...
    addFineControl('ShoulderWidthBasis', 'skeleton.shoulderWidth', ['shoulder_left', 'shoulder_right', 'chest', 'back', 'upper_arm_left', 'upper_arm_right'], ['torso', 'upperarm_l', 'upperarm_r'], (vx, _vy, _vz) => {
        const s = Math.sign(vx || 1e-6);
    ...
    addFineControl('SpineLengthBasis', 'skeleton.spineLength', ['chest', 'abdomen', 'back'], ['torso'], (_vx, vy, _vz) => ({ dx: 0, dy: (vy - 1.0) * 0.5, dz: 0 }), ['spine', 'correlated']);
    ...
    addFineControl('GlobalHeightBasis', 'global.height', HD_AXIAL_REGIONS_FINE, HD_AXIAL_REGIONS_COARSE, (_vx, vy, _vz) => ({ dx: 0, dy: vy, dz: 0 }), ['height', 'correlated']);
    ...
    const bodyCorrectiveBasis = (name, fineRegions, coarseRegions, fn) => {
        const finePresent = fineRegions.filter((r) => canonical.regionRanges.has(r));
        const coarsePresent = coarseRegions.filter((r) => canonical.regionRanges.has(r));
        const targets = finePresent.length >= coarsePresent.length ? finePresent : coarsePresent;
        if (targets.length === 0)
    ...
    if (muscleBasis && shoulderBasis) {
        const output = bodyCorrectiveBasis('MuscularBroadShouldersCorrective', ['shoulder_left', 'shoulder_right', 'chest', 'back', 'upper_arm_left', 'upper_arm_right'], ['torso', 'upperarm_l', 'upperarm_r'], (vx, _vy, _vz) => {
            const s = Math.sign(vx || 1e-6);
...
//# sourceMappingURL=hd-shape-builder.js.map