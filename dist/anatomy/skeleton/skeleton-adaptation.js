...
export const SYMMETRIC_BONE_PAIRS = [
...
export function boneWorldPositions(bones) {
 ...
 */
export function regionVertexIds(mesh, region) {
    const ids = [];
...
function positionAt(positions, id) {
    return { x: positions[id * 3], y: positions[id * 3 + 1], z: positions[id * 3 + 2] };
}
function anchorFrom(positions, ids, mode) {
    let cx = 0;
    let cy = 0;
    let cz = 0;
    let minY = Infinity;
 ...
 */
export function adaptSkeletonToPositions(bones, mesh, positions, options = {}) {
    ...
    const anchorByBone = new Map();
    for (const a of anchors)
        anchorByBone.set(a.bone, a);
    ...
    for (const b of out)
        outByName.set(b.name, b);
    const worldRot = new Map();
    const localize = (bone) => {
        if (worldRot.has(bone.name))
            return;
    ...
    for (const bone of bones) {
        ...
        const entry = { ...d, adaptedWorld: w, shift };
        joints.push(entry);
        sum += shift;
        if (shift > peak)
            peak = shift;
...
export function skeletonAdaptationReportLines(report) {
    const lines = [
    ...
    for (const j of report.joints) {
        lines.push(`  ${j.bone}: ${j.source}${j.region ? `(${j.region}:${j.vertexCount})` : ''} shift=${j.shift.toFixed(4)}${j.clamped ? ' CLAMPED' : ''}`);
    }
    return lines;
}
//# sourceMappingURL=skeleton-adaptation.js.map