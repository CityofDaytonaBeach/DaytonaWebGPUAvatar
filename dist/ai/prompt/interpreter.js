 ...
 */
export class DeterministicPromptInterpreter {
    interpret(prompt) {
        const p = prompt.toLowerCase();
        if (p.includes('smile'))
            return { type: 'expression', confidence: 0.9, expression: 'smile' };
        if (p.includes('frown'))
            return { type: 'expression', confidence: 0.9, expression: 'frown' };
        ...
        if (p.includes('sad'))
            return { type: 'expression', confidence: 0.9, expression: 'sad' };
 ...
 */
export function intentToEvent(intent, source = 'ai') {
    switch (intent.type) {
