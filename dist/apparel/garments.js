 ...
 */
export function garmentLayerForVertex(region, t, spec) {
    if (spec.style === 'none')
        return 'skin';
 ...
 */
export function splitBodyByGarment(canonical, bodyIndexEnd, spec) {
    const skin = [];
    const shirt = [];
    const pants = [];
    if (spec.style === 'none') {
    ...
    const layerOf = new Array(canonical.vertices.length);
