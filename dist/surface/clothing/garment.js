...
// ---------------------------------------------------------------------------
export function generateGarments(attachments, dims, options = {}) {
    return attachments.flatMap((attachment) => {
        if (attachment.kind !== 'wearable')
            return [];
        return [generateGarment(attachment, dims, options)];
    });
}
export function generateGarment(attachment, dims, options = {}) {
    if (attachment.kind !== 'wearable')
        throw new Error('Garments require a wearable attachment');
...
export function toRenderMesh(garment) {
...
export function toPhysicsMesh(garment, options = {}) {
...
export function simulateClothStep(physics, dt, solverIterations = 3) {
...
export function applyDrape(physics, bodySurface, attachmentRegions, dims, dt = 1 / 60, steps = 5) {
...
export function generateWrinkles(garment, dims, options = {}) {
    const frequency = options.frequency ?? 12;
    const amplitude = options.amplitude ?? 0.003;
    const seed = options.seed ?? 42;
    const offsets = garment.vertices.map((vt) => {
        ...
        return vec3((h - 0.5) * 2 * amplitude * intensity, (h2 - 0.5) * amplitude * intensity * 0.3, (h3 - 0.5) * 2 * amplitude * intensity);
    });
    return offsets;
}
/** Apply wrinkle offsets to a render mesh (mutates in place). */
export function applyWrinkles(renderMesh, offsets) {
...
export function generateGarmentLODs(attachment, dims, options = {}) {
...
export function selectLOD(distance, lodThresholds = [1.5, 4.0]) {
...
// ---------------------------------------------------------------------------
function makeShirt(id, dims, looseness, color) {
...
function makeSleeve(id, side, dims, looseness, color) {
...
function makePants(id, dims, looseness, color) {
    ...
    const indices = [];
    // Waistband front
    subdividedQuad(verts, indices, segments, vec3(-waist, top, depth), vec3(waist, top, depth), vec3(hip, knee, depth), vec3(-hip, knee, depth));
    // Waistband back
    subdividedQuad(verts, indices, segments, vec3(waist, top, -depth), vec3(-waist, top, -depth), vec3(-hip, knee, -depth), vec3(hip, knee, -depth));
    // Left leg front
    subdividedQuad(verts, indices, segments, vec3(-hip, knee, depth * 0.55), vec3(-thigh * 0.15, knee, depth * 0.55), vec3(-thigh * 0.15, ankle, depth * 0.35), vec3(-thigh, ankle, depth * 0.35));
    // Left leg back
    subdividedQuad(verts, indices, segments, vec3(-thigh * 0.15, knee, -depth * 0.55), vec3(-hip, knee, -depth * 0.55), vec3(-thigh, ankle, -depth * 0.35), vec3(-thigh * 0.15, ankle, -depth * 0.35));
    // Right leg front
    subdividedQuad(verts, indices, segments, vec3(thigh * 0.15, knee, depth * 0.55), vec3(hip, knee, depth * 0.55), vec3(thigh, ankle, depth * 0.35), vec3(thigh * 0.15, ankle, depth * 0.35));
    // Right leg back
    subdividedQuad(verts, indices, segments, vec3(hip, knee, -depth * 0.55), vec3(thigh * 0.15, knee, -depth * 0.55), vec3(thigh * 0.15, ankle, -depth * 0.35), vec3(thigh, ankle, -depth * 0.35));
...
function makeJacket(id, dims, looseness, color) {
    ...
    const lapGap = 0.008;
    subdividedQuad(verts, indices, segments, vec3(-chest, top, depth), vec3(-lapGap, top, depth), vec3(-waist, bottom, depth), vec3(-waist, bottom, depth));
    subdividedQuad(verts, indices, segments, vec3(lapGap, top, depth), vec3(chest, top, depth), vec3(waist, bottom, depth), vec3(waist, bottom, depth));
    // Back
    subdividedQuad(verts, indices, segments, vec3(chest, top, -depth), vec3(-chest, top, -depth), vec3(-waist, bottom, -depth), vec3(waist, bottom, -depth));
    // Left
    subdividedQuad(verts, indices, segments, vec3(-chest, top, -depth), vec3(-chest, top, depth), vec3(-waist, bottom, depth), vec3(-waist, bottom, -depth));
    // Right
    subdividedQuad(verts, indices, segments, vec3(chest, top, depth), vec3(chest, top, -depth), vec3(waist, bottom, -depth), vec3(waist, bottom, depth));
    // Top
    subdividedQuad(verts, indices, segments, vec3(-chest, top, -depth), vec3(chest, top, -depth), vec3(chest, top, depth), vec3(-chest, top, depth));
    return { id, kind: 'jacket', vertices: verts, indices: Uint32Array.from(indices), color };
}
function makeHat(id, dims, looseness, color) {
    ...
    for (let ly = 0; ly < layers; ly++) {
        ...
        for (let s = 0; s < segments; s++) {
            ...
            verts.push({ position: vec3(Math.cos(a0) * r0, y0, Math.sin(a0) * r0), uv: { u: u0, v: t0 } }, { position: vec3(Math.cos(a1) * r0, y0, Math.sin(a1) * r0), uv: { u: u1, v: t0 } }, { position: vec3(Math.cos(a1) * r1, y1, Math.sin(a1) * r1), uv: { u: u1, v: t1 } }, { position: vec3(Math.cos(a0) * r1, y1, Math.sin(a0) * r1), uv: { u: u0, v: t1 } });
    ...
    for (let s = 0; s < segments; s++) {
        ...
        verts.push({
            ...
            uv: { u: u1, v: 0 },
        }, { position: vec3(Math.cos(a1) * brimR, brimY, Math.sin(a1) * brimR), uv: { u: u1, v: 1 } }, { position: vec3(Math.cos(a0) * brimR, brimY, Math.sin(a0) * brimR), uv: { u: u0, v: 1 } });
        const base = brimBase + s * 4;
        indices.push(base, base + 1, base + 2, base, base + 2, base + 3);
    }
    return { id, kind: 'hat', vertices: verts, indices: Uint32Array.from(indices), color };
}
function makeShoes(id, dims, looseness, color) {
    ...
    const indices = [];
    // Top face
    subdividedQuad(verts, indices, segments, vec3(heelX, topY, -shoeW), vec3(toeX, topY, -shoeW), vec3(toeX, topY, shoeW), vec3(heelX, topY, shoeW));
    // Front face (toe cap)
    subdividedQuad(verts, indices, segments, vec3(toeX, topY, -shoeW), vec3(toeX, topY, shoeW), vec3(toeX, soleY, shoeW), vec3(toeX, soleY, -shoeW));
    // Back face (heel)
    subdividedQuad(verts, indices, segments, vec3(heelX, topY, shoeW), vec3(heelX, topY, -shoeW), vec3(heelX, soleY, -shoeW), vec3(heelX, soleY, shoeW));
    // Left
    subdividedQuad(verts, indices, segments, vec3(heelX, topY, -shoeW), vec3(toeX, topY, -shoeW), vec3(toeX, soleY, -shoeW), vec3(heelX, soleY, -shoeW));
    // Right
    subdividedQuad(verts, indices, segments, vec3(toeX, topY, shoeW), vec3(heelX, topY, shoeW), vec3(heelX, soleY, shoeW), vec3(toeX, soleY, shoeW));
    // Bottom
    subdividedQuad(verts, indices, segments, vec3(heelX, soleY, shoeW), vec3(toeX, soleY, shoeW), vec3(toeX, soleY, -shoeW), vec3(heelX, soleY, -shoeW));
...
function subdividedQuad(verts, indexOut, segments, tl, tr, br, bl) {
    ...
    for (let row = 0; row <= segments; row++) {
        ...
        for (let col = 0; col <= segments; col++) {
            ...
            const x = (1 - tu) * (1 - tv) * tl.x + tu * (1 - tv) * tr.x + tu * tv * br.x + (1 - tu) * tv * bl.x;
            const y = (1 - tu) * (1 - tv) * tl.y + tu * (1 - tv) * tr.y + tu * tv * br.y + (1 - tu) * tv * bl.y;
            const z = (1 - tu) * (1 - tv) * tl.z + tu * (1 - tv) * tr.z + tu * tv * br.z + (1 - tu) * tv * bl.z;
...
function decimateRenderMesh(mesh, fraction) {
...
function decimatePhysicsMesh(mesh, fraction) {
