...
export class SkeletalAnimation {
    ...
    sample(boneNames, t) {
        ...
        for (const bone of boneNames) {
            const out = { name: bone, localPos: { x: 0, y: 0, z: 0 }, localRot: IDENTITY_QUAT };
            for (const [clipName, channels] of this.clips) {
                ...
                if (!channel || channel.times.length === 0)
                    continue;
...
export function sampleChannel(channel, t) {
    ...
    if (t >= t1)
        return channel.rotations[channel.rotations.length - 1];
...
export function quatFromEulerDeg(xDeg, yDeg, zDeg) {
