import { quatFromEulerDeg, nlerp } from '../skeleton/skeletal-animation.js';
...
export class MotionCompiler {
    ...
    static compile(command, skeleton) {
...
export function compileMotionCommand(command, skeleton) {
 ...
 */
export function solveIK2Bone(chain, skeleton, config) {
    if (chain.bones.length < 2)
        return [];
    const byName = new Map(skeleton.map((b) => [b.name, b]));
    const bone0 = byName.get(chain.bones[0]);
    const bone1 = byName.get(chain.bones[1]);
    if (!bone0 || !bone1)
        return [];
 ...
 */
export function solveLookAt(skeleton, target, config) {
    const byName = new Map(skeleton.map((b) => [b.name, b]));
 ...
 * preserves rotations.
 */
export function retargetPoses(poses, mapping) {
    ...
    return poses.map((pose) => {
        const srcBone = srcByName.get(pose.name);
        const tgtBone = tgtByName.get(pose.name);
        if (!srcBone || !tgtBone)
            return pose;
 ...
 */
export function compileWalk(skeleton, phase, speed, _config) {
 ...
 */
export function blendMotions(from, to, t) {
 ...
 */
export function transitionTo(to, duration, config) {
    return {
        ...to,
        kind: 'transition',
 ...
 */
export function validateMotion(plan, skeleton) {
    const byName = new Map(skeleton.map((b) => [b.name, b]));
    const violations = [];
    for (const pose of plan.poses) {
        const bone = byName.get(pose.name);
        if (!bone || !bone.limits)
            continue;
...
function chainLength(bone, _config) {
...
function restPoses(skeleton, rotations) {
    const byName = new Map(skeleton.map((b) => [b.name, b]));
...
export function compileLookAt(skeleton, target, config) {
...
export function compileIKArm(skeleton, side, target, poleVector, config) {
    const cfg = { ...DEFAULT_CONFIG, ...config };
    const chain = {
...
export function compileIKLeg(skeleton, side, target, poleVector, config) {
    const cfg = { ...DEFAULT_CONFIG, ...config };
    const chain = {
...
//# sourceMappingURL=motion-compiler.js.map