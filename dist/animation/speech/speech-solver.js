...
export class SpeechSolver {
    ...
    poseAt(track, t) {
        ...
        for (const p of track.phonemes) {
            ...
            if (dist > p.duration / 2 + blendWindow)
                continue;
     ...
     */
    applyWithExpression(definition, track, t, expressionWeight) {
...
export function simpleTTS(text) {
    const chars = text.toLowerCase().split('');
    const phonemes = [];
    let t = 0;
    const perChar = 0.09;
    for (const ch of chars) {
        const viseme = ch === 'a' || ch === 'o'
            ? 'aa'
            : ch === 'e' || ch === 'i'
                ? 'ee'
                : ch === 'u'
...
//# sourceMappingURL=speech-solver.js.map