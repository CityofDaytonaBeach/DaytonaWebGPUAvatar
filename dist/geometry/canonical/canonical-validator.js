...
export const REQUIRED_CANONICAL_REGIONS = [
    'head',
    'face',
    'nose',
    'jaw',
...
export function validateCanonicalTopology(topology) {
    const issues = [];
...
export function validateCanonicalHuman(canonical) {
    const topology = {
...
function validateVertices(vertices, issues) {
    const seen = new Set();
...
function validateIndices(topology, issues) {
    const vertexCount = topology.vertices.length;
...
function validateRegions(topology, issues) {
    ...
    for (const region of REQUIRED_CANONICAL_REGIONS) {
        ...
        if (fines && fines.every((fine) => counts.get(fine)))
            continue;
        issues.push({ code: 'missing-region', message: `missing required region ${region}` });
    }
}
function validateParts(topology, vertexCount, issues) {
...
function validatePart(part, vertexCount, indexCount, issues) {
    if (part.vertexStart < 0 ||
        part.vertexCount <= 0 ||
...
function validatePartRegionCoverage(topology, part, issues) {
...
function overlap(startA, countA, startB, countB) {
    return startA < startB + countB && startB < startA + countA;
}
function regionNames(vertices) {
    const names = new Set();
...
//# sourceMappingURL=canonical-validator.js.map