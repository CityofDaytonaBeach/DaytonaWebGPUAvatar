...
export function seedTurbulence(seed) {
    _turbSeed = seed | 0 || 1;
}
/** Build a deterministic poncho/shirt-front cloth panel pinned near shoulders. */
export function createTorsoCloth(width = 8, height = 10) {
    const particles = [];
    const constraints = [];
    const dx = 0.08;
    const dy = 0.07;
...
export function stepCloth(mesh, sdf, options = {}) {
    const config = {
        ...DEFAULT_CONFIG,
        ...options,
        wind: DEFAULT_WIND,
...
export function stepClothAdvanced(mesh, sdf, config) {
    ...
    for (const p of next.particles) {
        ...
        p.position = vec3(p.position.x + vx + fx * dt * dt, p.position.y + vy + fy * dt * dt, p.position.z + vz + fz * dt * dt);
        p.previous = pos;
    }
...
export function simulateCloth(mesh, sdf, steps, options = {}) {
    let current = mesh;
    for (let i = 0; i < steps; i++)
        current = stepCloth(current, sdf, options);
    return current;
}
export function simulateClothAdvanced(mesh, sdf, steps, config) {
    let current = mesh;
    for (let i = 0; i < steps; i++)
        current = stepClothAdvanced(current, sdf, config);
    return current;
}
export function cloneCloth(mesh) {
    return {
        width: mesh.width,
        height: mesh.height,
...
export function clothToGPUBuffer(mesh) {
...
export function clothConstraintsToGPUBuffer(mesh) {
...
export function clothRestLengthsToGPUBuffer(mesh) {
...
export function meshToGPULayout(mesh) {
    const n = mesh.particles.length;
...
function satisfyConstraints(mesh, stiffness, tearThreshold, live) {
...
function collidePrimitives(mesh, primitives, padding) {
    for (const p of mesh.particles) {
        if (p.pinned)
            continue;
...
function sphereCollide(p, prim, padding) {
...
function capsuleCollide(p, prim, padding) {
    const a = prim.center;
...
function avoidSelfCollision(mesh, radius) {
...
//# sourceMappingURL=cloth-sim.js.map