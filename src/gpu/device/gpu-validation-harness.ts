...
export function resolveLimits(limits?: GpuLimitsLike): GpuLimits {
...
export function validateDispatch(spec: DispatchSpec, limits?: GpuLimitsLike): GpuBoundsReport {
  ...
  if (!Number.isInteger(spec.workgroupSize) || spec.workgroupSize <= 0) {
    ...
    push(
      'dispatch.workgroup-size-exceeds-limit',
      `workgroupSize ${spec.workgroupSize} > maxComputeInvocationsPerWorkgroup ${l.maxComputeInvocationsPerWorkgroup}`,
    );
  }
  ...
  spec.workgroups.forEach((count, i) => {
    if (!Number.isInteger(count) || count < 0) {
      push(
        'dispatch.invalid-workgroup-count',
        `workgroups.${axes[i]} must be a non-negative integer`,
      );
      return;
    }
    if (count > l.maxComputeWorkgroupsPerDimension) {
      push(
        'dispatch.exceeds-limit',
        `workgroups.${axes[i]} ${count} > maxComputeWorkgroupsPerDimension ${l.maxComputeWorkgroupsPerDimension}`,
      );
    }
  });

  const total =
    spec.workgroups[0] * spec.workgroups[1] * spec.workgroups[2] * Math.max(1, spec.workgroupSize);
  if (spec.workItems > 0 && total < spec.workItems) {
    push(
      'dispatch.under-covers-work',
      `dispatch covers ${total} invocations but ${spec.workItems} work items exist (mesh would be partially deformed)`,
    );
  }
  ...
  if (spec.workItems > 0 && total >= spec.workItems + spec.workgroupSize) {
    push(
      'dispatch.over-covers-work',
      `dispatch covers ${total} invocations for ${spec.workItems} work items (>=1 fully wasted workgroup)`,
    );
  }
...
export function validateBufferBinding(
  ...
  const push = (
    code: string,
    message: string,
    severity: GpuValidationSeverity = 'validation',
  ): void => {
    issues.push({ code, message, severity, scope: spec.scope });
  };
  const offset = spec.offset ?? 0;

  if (spec.byteSize <= 0) push('buffer.empty', `${spec.label}: buffer has zero size`);
  if (offset % 4 !== 0)
    push('buffer.misaligned-offset', `${spec.label}: offset ${offset} not 4-byte aligned`);
  if (spec.strideBytes <= 0 || spec.strideBytes % 4 !== 0) {
    push(
      'buffer.invalid-stride',
      `${spec.label}: stride ${spec.strideBytes} must be a positive multiple of 4`,
    );
  }
  ...
  if (spec.maxElementIndex >= 0 && required > spec.byteSize) {
    push(
      ...
      `${spec.label}: shader reads up to byte ${required} but buffer is ${spec.byteSize} bytes`,
    );
  }

  const cap =
    spec.kind === 'uniform' ? l.maxUniformBufferBindingSize : l.maxStorageBufferBindingSize;
  if (spec.byteSize > cap) {
    push(
      'buffer.exceeds-binding-limit',
      `${spec.label}: ${spec.byteSize} bytes > ${spec.kind === 'uniform' ? 'maxUniformBufferBindingSize' : 'maxStorageBufferBindingSize'} ${cap}`,
      'out-of-memory',
    );
  }
  if (spec.byteSize > l.maxBufferSize) {
    push(
      ...
      `${spec.label}: ${spec.byteSize} bytes > maxBufferSize ${l.maxBufferSize}`,
      'out-of-memory',
    );
  }
 ...
 */
export function validatePackedMorphBounds(
  scope: string,
  ...
  for (let i = 0; i < morphCount; i++) {
    ...
    if (offset + count > slots) {
      push(
        'morph.range-out-of-bounds',
        `morph ${i}: range [${offset}, ${offset + count}) exceeds ${slots} delta slots`,
      );
    }
  }
  ...
  for (let s = 0; s < slots; s++) {
    ...
    if (index >= vertexCount) {
      push(
        'morph.vertex-index-out-of-bounds',
        `delta slot ${s}: vertex ${index} >= vertexCount ${vertexCount}`,
      );
...
export class GpuValidationHarness {
  private readonly captured: GpuValidationIssue[] = [];

  constructor(private readonly device: ErrorScopeCapableDevice) {}
  ...
  async capture<T>(scope: string, work: () => T | Promise<T>): Promise<T> {
    ...
    try {
      ...
      if (validation) this.record(scope, validation, 'validation');
      if (oom) this.record(scope, oom, 'out-of-memory');
    }
    return result;
  }
  ...
  assertClean(): void {
    ...
    throw new Error(
      `GPU validation failed (${this.captured.length}):\n` +
  ...
  private record(scope: string, error: GPUError, severity: GpuValidationSeverity): void {
    this.captured.push({
