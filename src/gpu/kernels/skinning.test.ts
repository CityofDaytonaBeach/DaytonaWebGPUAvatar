...
describe('CPU skinning', () => {
  ...
  it('rest pose preserves the base geometry exactly', () => {
    ...
    const bones = placeSkeletonFromDefinition(
  ...
  it('rotating thigh_l moves only its FK chain (thigh_l + shin_l), never the other side', async () => {
    ...
    const poses = [
      { name: 'thigh_l', localPos: { x: 0, y: 0, z: 0 }, localRot: quatFromEulerDeg(30, 0, 0) },
    ];
    human.setPose(poses);
...
describe('GPU/CPU skinning parity', () => {
  ...
  function simulateGpu(
    base: Float32Array,
    influences: ReturnType<typeof buildInfluences>,
    mats: Float32Array,
  ): Float32Array {
    ...
    for (let v = 0; v < n; v++) {
      ...
      for (let k = 0; k < 4; k++) {
        ...
        const sx =
          mats[bi + 0] * base[v * 3] +
          mats[bi + 4] * base[v * 3 + 1] +
          mats[bi + 8] * base[v * 3 + 2] +
          mats[bi + 12];
        const sy =
          mats[bi + 1] * base[v * 3] +
          mats[bi + 5] * base[v * 3 + 1] +
          mats[bi + 9] * base[v * 3 + 2] +
          mats[bi + 13];
        const sz =
          mats[bi + 2] * base[v * 3] +
          mats[bi + 6] * base[v * 3 + 1] +
          mats[bi + 10] * base[v * 3 + 2] +
          mats[bi + 14];
        x += w * sx;
        y += w * sy;
        z += w * sz;
