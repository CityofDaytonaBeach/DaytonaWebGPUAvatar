...
import {
  ...
  type MeasurementLandmarkName,
  type ProportionReport,
} from '../reference/anthropometry.js';
 ...
 */
export const SHIPPABLE_LICENSES: readonly string[] = Object.freeze([
  'mit',
  'apache-2.0',
  'apache2',
  'bsd-2-clause',
 ...
 */
export const RESEARCH_ONLY_LICENSES: readonly string[] = Object.freeze([
  'cc-by-nc-4.0',
  'cc-by-nc-sa-4.0',
  'cc-by-nc-nd-4.0',
  'flame-license',
...
export function assetLicenseClass(license: string): LicenseClass {
  const id = license.trim().toLowerCase();
  if (SHIPPABLE_LICENSES.includes(id)) return 'shippable';
  if (RESEARCH_ONLY_LICENSES.includes(id)) return 'research_only';
  return 'unknown';
}

/** True only for licenses that may be redistributed in a commercial build. */
export function isShippableLicense(license: string): boolean {
  return assetLicenseClass(license) === 'shippable';
}

// ---------------------------------------------------------------------------
// The neutral asset format
// ---------------------------------------------------------------------------
...
const finite3 = (arr: Float32Array): boolean => {
 ...
 */
export function inspectBaseHeadAsset(asset: BaseHeadAsset): BaseHeadInspection {
  const issues: BaseHeadValidationIssue[] = [];
...
export function measureAsset(asset: BaseHeadAsset): ProportionReport {
  const positions: LandmarkPositions = {};
  for (const lm of asset.landmarks ?? []) {
    const i = lm.vertexIndex * 3;
    if (i + 2 >= asset.positions.length) continue;
    positions[lm.name as MeasurementLandmarkName] = [
      asset.positions[i],
      asset.positions[i + 1],
      asset.positions[i + 2],
    ];
...
export function computeAssetNormals(positions: Float32Array, indices: Uint32Array): Float32Array {
...
export class BaseHeadLicenseError extends Error {
  constructor(
    ...
    super(
      `Base head "${assetName}" is licensed "${license}" (${licenseClass}) and cannot be baked into a ` +
 ...
 */
export class BaseHeadProvider implements CanonicalHumanProvider {
  readonly version = 'DaytonaCanonicalHuman v0.1';
  private readonly headBone: string;
  private readonly neckBone: string;
  private readonly requireShippable: boolean;
  private readonly requireProportions: boolean;

  constructor(
    private readonly asset: BaseHeadAsset,
    opts: BaseHeadProviderOptions = {},
  ) {
  ...
  private build(): CanonicalHumanAsset {
    ...
    if (!inspection.valid) {
      throw new Error(
        `Base head "${this.asset.name}" is structurally invalid: ` +
    ...
    if (this.requireProportions && inspection.proportions && !inspection.proportions.pass) {
      throw new Error(
        `Base head "${this.asset.name}" fails the anthropometric canons: ` +
    ...
    const vertices: CanonicalTopologyVertex[] = new Array(vertexCount);
    for (let v = 0; v < vertexCount; v++) {
    ...
    const parts: CanonicalTopologyPart[] = [
      {
        name: 'base_head_skin',
        kind: 'skin',
        region: 'head',
    ...
    const topology: CanonicalTopology = { vertices, indices, parts };
    ...
    return {
      ...
      metadata: {
        ...
        note:
          `scan-derived base head; source=${this.asset.source}; license=${this.asset.license} ` +
   ...
   */
  private surfaceLandmarks(indices: Uint32Array, vertexCount: number): HumanLandmark[] {
    ...
    for (let t = 0; t + 2 < indices.length; t += 3) {
      for (let c = 0; c < 3; c++) {
        ...
        if (firstTriangleFor[v] === -1) {
          ...
          cornerFor[v] = c as 0 | 1 | 2;
        }
      }
    }
    const out: HumanLandmark[] = [];
    let id = 0;
