 ...
 */
export const COARSE_REGION_FINE_ALIASES: Partial<Record<RegionName, RegionName[]>> = {
 ...
 */
export function buildRegionRanges(
  vertices: ReadonlyArray<{ region: RegionName }> | ReadonlyArray<{ readonly region: RegionName }>,
): Map<RegionName, IndexRange> {
  const ranges = new Map<RegionName, IndexRange>();
  ...
  for (const [coarse, fines] of Object.entries(COARSE_REGION_FINE_ALIASES) as Array<
    [RegionName, RegionName[]]
  >) {
    if (ranges.has(coarse) || !fines) continue;
    let start = -1;
    let count = 0;
    for (const fine of fines) {
      const range = ranges.get(fine);
      if (!range) continue;
      if (start === -1 || range.start < start) start = range.start;
      count += range.count;
    }
    if (start !== -1) ranges.set(coarse, { start, count });
  }
