...
function normWeights(w: Record<string, number>): Record<string, number> {
...
interface BodyColumn {
  ...
  quad: Array<[number, number, number, number, number, number]>;
}
...
function boneOnly(name: string): Record<string, number> {
  return { [name]: 1 };
}
 ...
 */
export function buildHdBodySkin(opts: HdBodySkinOptions = {}): {
  vertices: CanonicalTopologyVertex[];
  indices: Uint32Array;
} {
    ...
    for (let i = 0; i < rows; i++) {
      ...
      for (let j = 0; j < W; j++) {
        ...
        if (p.y >= 1.5 && lateral > spec.rx * 0.72) {
          ...
          let region: RegionName = spec.region;
  ...
  // ------------------------------------------------------------------ arms
  const buildArm = (side: 1 | -1): BodyColumn => {
    const l = side < 0;
  ...
  // ------------------------------------------------------------------ legs
  const buildLeg = (side: 1 | -1): BodyColumn => {
    const l = side < 0;
    const prefix = (n: string) => n + (l ? '_l' : '_r');
    const ox = side * 0.075;
  ...
  // ------------------------------------------------------------------ feet
  const buildFoot = (side: 1 | -1): BodyColumn => {
    const l = side < 0;
    const prefix = (n: string) => n + (l ? '_l' : '_r');
    const ringR = 0.05;
