...
export const JAW_DRIVEN_REGIONS: RegionName[] = [
  'jaw_left',
  'jaw_right',
  'chin',
  'upper_lip',
...
export function headSkinWeights(
  region: RegionName,
  headBone = 'head',
  neckBone = 'neck',
): Record<string, number> {
  if (region === 'neck') return { [neckBone]: 1.0 };
 ...
 */
export function headRegionFor(y: number, x: number, z: number): RegionName {
...
export const REGION_ANCHORS: Partial<Record<RegionName, { x: number; y: number; z: number }>> = {
 ...
 */
export function ensureHeadRegions<
  T extends { region: RegionName; position: { x: number; y: number; z: number } },
