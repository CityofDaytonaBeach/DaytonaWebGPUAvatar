...
import { Vec3, clamp01, vdot, vlerp, vnormalize, vscale } from './color.js';
 ...
 */
export function irisParallaxOffset(
  normal: Vec3,
  viewDir: Vec3,
  ior = 1.376,
  irisRadius = 0.006,
  corneaDepth = PHOTOREAL_CONSTANTS.corneaDepth,
): { du: number; dv: number } {
...
export function pupilRadius(sceneLuminance: number, dilation = 0.5): number {
...
export function limbalRing(radius: number): number {
 ...
 */
export function shadeSclera(baseColor: Vec3, cornerness: number, vascularColor?: Vec3): Vec3 {
 ...
 */
export function enamelFactor(edgeProximity: number, archDepth: number): number {
...
export function shadeEnamel(baseColor: Vec3, edgeProximity: number, archDepth: number): Vec3 {
  const factor = enamelFactor(edgeProximity, archDepth);
  const cool: Vec3 = [0.72, 0.76, 0.82];
