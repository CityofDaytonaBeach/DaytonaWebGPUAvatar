...
export function buildOneRing(indices: Uint32Array, vertexCount: number): Uint32Array[] {
 ...
 */
export function bakeCurvature(vertices: readonly Vertex[], indices: Uint32Array): Float32Array {
...
class VertexGrid {
  ...
  constructor(
    private readonly vertices: readonly Vertex[],
 ...
 */
export function bakeThickness(vertices: readonly Vertex[]): Float32Array {
  const n = vertices.length;
  const out = new Float32Array(n);
  ...
  for (let i = 0; i < n; i++) {
    const v = vertices[i];
    let best: number = C.thicknessMax;
    for (let s = 1; s <= steps; s++) {
      ...
      for (const j of grid.near(px, py, pz)) {
        ...
        if (dx * dx + dy * dy + dz * dz > cell * cell) continue;
...
export function bakeCurvatureThickness(canonical: CanonicalHuman): CurvatureThicknessBake {
