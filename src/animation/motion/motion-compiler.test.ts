...
const skeleton = placeSkeletonFromDefinition(resolveAnatomy(new HumanDefinition(createDefaultRegistry())));
...
describe("motion compiler", () => {
  it("compiles a hand-raise command into rest-preserving arm poses", () => {
    ...
    expect(plan.poses.map((p) => p.name)).toEqual(["clavicle_r", "upperarm_r", "forearm_r", "hand_r"]);
    expect(plan.poses[1].localPos).toEqual(skeleton.find((b) => b.name === "upperarm_r")!.localPosition);
  });

  it("compiles look-at-camera and neutral commands", () => {
    expect(compileMotionCommand("look toward the camera", skeleton).poses.map((p) => p.name)).toEqual(["neck", "head"]);
