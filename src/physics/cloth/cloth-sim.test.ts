...
describe('cloth simulation prototype', () => {
  ...
  it('moves unpinned particles under gravity while preserving pinned anchors', async () => {
    ...
    const stepped = stepCloth(cloth, human.sdfField(), {
      dt: 1 / 30,
      iterations: 3,
      collisionPadding: 0,
    });
  ...
  it('projects cloth particles out of the human SDF collision field', async () => {
    ...
    const simulated = human.simulateCloth(cloth, 4, {
      dt: 1 / 60,
      iterations: 6,
      collisionPadding: 0.01,
    });
    const minDistance = Math.min(
      ...simulated.particles.filter((p) => !p.pinned).map((p) => human.sdfDistance(p.position)),
    );
  ...
  it('responds to larger bodies through the same SDF collision path', async () => {
    ...
    const leanAvgZ =
      leanSim.particles.reduce((sum, p) => sum + p.position.z, 0) / leanSim.particles.length;
    const largeAvgZ =
      largeSim.particles.reduce((sum, p) => sum + p.position.z, 0) / largeSim.particles.length;
