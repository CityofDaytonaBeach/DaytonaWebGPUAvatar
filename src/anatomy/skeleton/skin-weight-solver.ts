...
export const REGION_BONE_PRIOR: Partial<Record<RegionName, BoneName>> = {
  head: 'head',
  face: 'head',
  forehead: 'head',
  temple_left: 'head',
 ...
 */
export function buildBoneSegments(bones: BoneDef[]): BoneSegment[] {
  const world = boneWorldPositions(bones);
  const childrenOf = new Map<BoneName, BoneName[]>();
...
export function distanceToSegment(p: Vec3, s: BoneSegment): number {
  const abx = s.b.x - s.a.x;
  const aby = s.b.y - s.a.y;
  const abz = s.b.z - s.a.z;
  const denom = abx * abx + aby * aby + abz * abz;
  if (denom < EPS) return distance(p, s.a);
 ...
 */
export function solveSkinWeights(
  mesh: CanonicalHuman,
  bones: BoneDef[],
  positions?: Float32Array,
  options: SkinWeightOptions = {},
): { weights: SkinWeightSet; report: SkinWeightReport } {
  ...
  const weights: SkinWeightSet = new Map();
  ...
  for (const vertex of mesh.vertices) {
    ...
    for (const seg of segments) {
      ...
      if (previous === undefined || score > previous) raw.set(seg.bone, score);
    }
    if (prior) {
    ...
    if (Math.abs(check - 1) > 1e-6) sumErrors += 1;
...
export function applySkinWeights(mesh: CanonicalHuman, weights: SkinWeightSet): number {
  let applied = 0;
  for (const vertex of mesh.vertices) {
...
export function validateSkinWeights(
  mesh: CanonicalHuman,
  bones: BoneDef[],
  weights: SkinWeightSet,
  maxInfluences = 4,
): SkinWeightValidation {
  const known = new Set(bones.map((b) => b.name as string));
  ...
  for (const vertex of mesh.vertices) {
    ...
    if (entries.length > maxInfluences) overBudget += 1;
    let sum = 0;
    for (const [bone, value] of entries) {
      if (!known.has(bone)) unknown.add(bone);
      sum += value;
    }
    if (Math.abs(sum - 1) > 1e-6) badSum += 1;
  }

  const issues: string[] = [];
  if (missing > 0) issues.push(`${missing} vertices have no weights`);
  if (overBudget > 0) issues.push(`${overBudget} vertices exceed ${maxInfluences} influences`);
  if (badSum > 0) issues.push(`${badSum} vertices do not sum to 1`);
  if (unknown.size > 0) issues.push(`unknown bones: ${[...unknown].sort().join(', ')}`);
