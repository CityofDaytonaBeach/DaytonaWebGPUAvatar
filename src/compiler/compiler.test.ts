...
describe('DependencyGraph', () => {
  ...
  it('groups affected properties into semantic human systems', () => {
    const { registry } = makeHuman();
    const graph = new DependencyGraph(registry);
    const nose = affectedSystemsForChange(registry, graph, [
      registry.require('face.nose.width').id,
    ]);
    const hair = affectedSystemsForChange(registry, graph, [registry.require('hair.length').id]);
    const height = affectedSystemsForChange(registry, graph, [
      registry.require('global.height').id,
    ]);
...
describe('DeltaCompiler', () => {
  ...
  it('assigns localized vertex ranges when canonical ranges are available', () => {
    const { registry } = makeHuman();
    const graph = new DependencyGraph(registry);
    const canonical = new CanonicalHuman([
      'root',
      'pelvis',
      'spine_01',
      'spine_02',
    ...
    const noseWork = delta
      .compile([registry.require('face.nose.width').id])
      .find((work) => work.kind === 'SparseMorph')!;
    const bodyWork = delta
      .compile([registry.require('body.muscularity').id])
      .find((work) => work.kind === 'SparseMorph')!;
...
describe('Timeline + Identity', () => {
  ...
  it('identity solver allows explicitly targeted structural edits', () => {
    ...
    const gate = solver.gate(
      createEvent('set', 'ui', { path: 'face.nose.width', value: 0.5 }),
      def,
    );
    expect(gate.allowed).toBe(true);
    // Expression is never identity-affecting.
    const exprGate = solver.gate(
      createEvent('set', 'ui', { path: 'expression.mouthSmileLeft', value: 1 }),
      def,
    );
